...
export default function Page() {
    ...
    const handleEditTask = (taskInputId) => {
        const taskInputField = document.getElementById(taskInputId);
        taskInputField.disabled = false;
        setEditId(taskInputId)

    }

    const handleSaveTask = async (id) => {
        const taskInputField = document.getElementById(`task-input-${id}`);
        const edit = await axios.patch('http://localhost:8080/tolist', { id, task: taskInputField.value })
        setEditId("")
        getApiHandle();
    ...
    // }
    return (<>
        <div className='container-fluid  text-center bg-purple-300'>
                            ...
                            <div key={v?.id} className='mt-10 bg-gray-300 p-2 mx-2'>
                                <div className='flex flex-wrap justify-between px-2'>
                                    ...
                                    <div className="flex gap-2 w-100">
                                        ...
                                        <div className="flex-1">
                                            <button onClick={() => editId.length ? handleSaveTask(v.id) : handleEditTask(`task-input-${v.id}`)}

                                                className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-md">
                                                <svg className="h-5 w-8 text-white-500" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z" />  <path d="M9 7 h-3a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-3" />  <path d="M9 15h3l8.5 -8.5a1.5 1.5 0 0 0 -3 -3l-8.5 8.5v3" />  <line x1="16" y1="5" x2="19" y2="8" /></svg>
                                                {editId == `task-input-${v.id}` ? "Save" : "Edit"}
                                            </button>
                                        </div>
                                    </div>
                                </div>
